...
const contactInfo = graduates.map ((name) => {
  return (`${name.first_name} ${name.email}`);
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter((name) => {
  return name.university.match("Uni");
})
...
*/
const lowPopulationAnimals = zooAnimals.filter((amount) => {
  return (amount.population < 5);
});
...
*/
const populationTotal = zooAnimals.reduce((total, tallied) => {
  return (total += tallied.population);
}, 0);
